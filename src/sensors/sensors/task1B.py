...
class PickPlace(Node):
    ...
    def upd(self):
        ...
        if p == 0:
            ...
            self.log_phase(f"Phase 16: Moving to trash bin with fixed quaternion")
            ...
            self.log_phase("Phase 18: Moving to SAFE retreat pose")
            ...
            self.log_phase("Phase 19: Moving to HOME position")
            ...
            if fert_pos is not None:
                # ✅ HARDCODED: Assume can's axis when on eBot
                z_axis = np.array([0.0, 0.0, -1.0])
                
                offset = 0.15
                target_pos = fert_pos - offset * z_axis
                
                self.log_phase("Phase 100: Moving to fertilizer approach (2nd time)")
        ...
        # elif p == 102:
        #     # ✅ Use stored hardcoded axis
        #     z_axis = self.fert_z
            
        #     offset = 0.15
        #     target_pos = self.fert_pos - offset * z_axis
        #     target_quat = self.calc_ee_orientation(z_axis, for_pick=True)

        #     self.log_phase("Phase 101: Rotating for fertilizer (2nd time)")
        ...
        #         self.st = time.time()
            ...
            self.log_phase("Phase 102: Grasping fertilizer (2nd time)")

            if self.servo_pose(target_pos, target_quat):
                self.ph = 103
                self.st = time.time()
        elif p == 103:
            self.log_phase("Phase 103: Attaching fertilizer can")
            if self.wait(0.3):
                success = self.attach('fertiliser_can')
                if success:
                    self.get_logger().info("✅ Fertilizer can attached successfully")
                else:
                    self.get_logger().warn("⚠️ Attach may have failed, continuing anyway")
                self.ph = 104
                self.st = time.time()
        ...
        elif p == 104.5:  # NEW PHASE: Virtual Base Rotation 90 deg
            if not hasattr(self, 'vb_rot_initialized'):
                # Get current EE pose
                eef_pos, eef_quat = self.get_eef_pose()
                if eef_pos is not None:
                    self.R_cur = R.from_quat(eef_quat)
                    self.p_cur = eef_pos.copy()
                    
                    # Desired rotation: 90 deg about base Z-axis
                    self.desired_base_rot = -np.pi / 2  # negative for clockwise
                    Rz = R.from_rotvec([0, 0, self.desired_base_rot])
                    
                    # Compute desired EE pose after virtual base rotation
                    self.p_des = Rz.apply(self.p_cur)
                    self.R_des = Rz * self.R_cur
                    
                    self.K_pos_vb = 1.2
                    self.K_rot_vb = 1.5
                    self.max_lin_vb = 0.5
                    self.max_ang_vb = 1.5
                    self.pos_tol_vb = 0.01
                    self.rot_tol_vb = 0.02
                    
                    self.vb_rot_initialized = True
                    self.get_logger().info("Phase 104.5: Virtual base rotation started")

            if hasattr(self, 'vb_rot_initialized'):
                # Get current EE pose
                eef_pos, eef_quat = self.get_eef_pose()
                if eef_pos is None:
                    return
                
                R_cur = R.from_quat(eef_quat)
                pos_err = self.p_des - eef_pos
                pos_dist = np.linalg.norm(pos_err)
                
                rot_err = self.R_des * R_cur.inv()
                rot_vec = rot_err.as_rotvec()
                rot_dist = np.linalg.norm(rot_vec)
                
                # Check if rotation is complete
                if pos_dist < self.pos_tol_vb and rot_dist < self.rot_tol_vb:
                    self.stop_t()
                    self.get_logger().info("✅ Virtual base rotation completed")
                    delattr(self, 'vb_rot_initialized')
                    self.ph = 105
                    self.st = time.time()
                else:
                    # Compute velocity
                    lin_vel = np.clip(self.K_pos_vb * pos_err, -self.max_lin_vb, self.max_lin_vb)
                    ang_vel = np.clip(self.K_rot_vb * rot_vec, -self.max_ang_vb, self.max_ang_vb)
