...
class LiDARRansacVisualizer(Node):
    ...
    def __init__(self):
        ...
        # Timing control for logging rate limiting
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        if self.should_log():
            self.get_logger().info(f"[SCAN] Total ranges={len(msg.ranges)}, Valid points={len(points)}")
        ...
        if now - self.last_display_time >= self.display_interval:
            ...
            if self.should_log():
                self.get_logger().info(
                    f"[PROCESS] detection_enabled={self.detection_enabled}, points={len(points)}"
                )
    ...
    def process_and_visualize(self, points):
        ...
        if len(ang_pos) > 0:
            ...
            if self.should_log():
                self.get_logger().info(
                    f"[CLUSTER +] n_clusters={n_clusters_pos}, points={len(ang_pos)}"
                )
            ...
            for i, (xc, yc, ac) in enumerate(clusters_pos):
                ...
                if result and result["n_points"] >= 30:
                    ...
                    if self.should_log():
                        self.get_logger().info(
                            f"[LINE +] slope={result['slope']:.2f}, "
                            f"points={result['n_points']}, vertical={result['vertical']}"
                        )
        ...
        if self.should_log():
            self.get_logger().info(
                f"[LINES] pos={len(lines_pos)}, neg={len(lines_neg)}"
            )
                ...
                for j in range(i + 1, len(lines_pos)):
                    ...
                    if 30 <= angle <= 80:
                        ...
                        self.get_logger().warn(
                            f"[DETECTED] TRIANGLE at ({avg_x:.2f}, {avg_y:.2f})"
                        )
                        break
                if detected_shape:
                    break
        ...
        if not detected_shape and len(lines_pos) >= 2:
            for i in range(len(lines_pos)):
                for j in range(i + 1, len(lines_pos)):
                    ...
                    if self.should_log():
                        self.get_logger().info(
                            f"[SQUARE CHECK] angle={angle:.2f}°, intercept_diff={intercept_diff:.2f}"
                        )
                    if (0 <= angle <= 20 or 80 <= angle <= 90) and intercept_diff > 0.2:
                        detected_shape = "SQUARE"
                        shape_position = (avg_x, avg_y)
                        self.get_logger().warn(
                            f"[DETECTED] SQUARE at ({avg_x:.2f}, {avg_y:.2f})"
                        )
                        break
                if detected_shape:
                    break
        ...
        if detected_shape and shape_position:
            ...
            if self.should_log():
                self.get_logger().info(
                    f"[CONFIRM] history size={len(self.shape_detection_history)}"
                )
            ...
            if len(self.shape_detection_history) >= self.confirmation_threshold:
                ...
                if len(set(shape_types)) == 1:
                    ...
                    if all_similar:
                        ...
                        self.get_logger().warn(
                            f"[PUBLISH] Confirmed {detected_shape} at ({avg_x:.2f}, {avg_y:.2f})"
                        )
